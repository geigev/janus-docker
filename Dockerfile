...
# init build env & install apt deps
    ...
    && export JANUS_WITH_LUA="${JANUS_WITH_LUA}"\
    ...
    && if [ $JANUS_WITH_LUA = "0" ]; then export JANUS_CONFIG_OPTIONS="$JANUS_CONFIG_OPTIONS --disable-plugin-lua"; fi \
...
# build usrsctp
    ...
    ; fi \
# build libwebsockets
    && if [ $JANUS_WITH_WEBSOCKETS = "1" ]; then git clone https://github.com/warmcat/libwebsockets.git --branch v2.3-stable ${BUILD_SRC}/libwebsockets \
